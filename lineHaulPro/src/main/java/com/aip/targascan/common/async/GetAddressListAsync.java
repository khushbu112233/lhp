...
public class GetAddressListAsync extends AsyncTask<Void, Void, String> {
	...
	public GetAddressListAsync(Context context, boolean isProgressBar, ICallback callBack) {
	...
	protected void onPreExecute() {
		try {
			if (isProgressBar)
				dialog = ProgressDialog.show(mcontext, "Getting Addresses...", "Loading");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	protected String doInBackground(Void... param) {
		try {
			...
			Log.e("MainUrl", "" + JsonKey.getADDRESS_LIST());
			String response = reader.sendRequest(JsonKey.getADDRESS_LIST(),params);
			return response;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
