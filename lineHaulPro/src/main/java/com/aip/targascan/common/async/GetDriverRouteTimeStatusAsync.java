...
public class GetDriverRouteTimeStatusAsync extends AsyncTask<Void, Void, String> {
	...
	public GetDriverRouteTimeStatusAsync(Context context, boolean isProgressBar, ICallback callBack) {
	...
	protected void onPreExecute() {
		try {
			if (isProgressBar)
				dialog = ProgressDialog.show(mcontext, "Getting Driver Route Time Status...", "Loading");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	protected String doInBackground(Void... param) {
		try {
			...
			Log.e("MainUrl", "" + JsonKey.getDriver_Route_Status());
			String response = reader.sendRequest(JsonKey.getDriver_Route_Status(), params);
			return response;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
